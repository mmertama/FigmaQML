...
public:
    ...
    virtual std::optional<std::tuple<QByteArray, int>> cachedImage(const QString& imageRef);
    virtual std::optional<std::tuple<QByteArray, int>> cachedRendering(const QString& figmaId);
    virtual std::optional<QByteArray> cachedNode(const QString& figmaId);
...
signals:
    void imageReady(const QString& imageRef, QByteArray& bytes, int format);
