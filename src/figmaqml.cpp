...
bool FigmaQml::saveAllQML(const QString &folderName) const {
    ...
    for(const auto& c : *m_sourceDoc) {
        for(const auto& e : *c) {
            const auto fullname = QString("%1/%2_%3.qml").arg(d.absolutePath()).arg(validFileName(c->name())).arg(validFileName(e->name()));
            QFile file(fullname);
...
FigmaQml::FigmaQml(const QString& qmlDir, const ImageProvider& byteProvider, const NodeProvider& dataProvider, QObject *parent) : QObject(parent),
    m_qmlDir(qmlDir), mImageProvider(byteProvider), mNodeProvider(dataProvider), m_imports(defaultImports()) {
...
QVariantMap FigmaQml::defaultImports() {
    return {
        {"QtQuick", QString("2.14")},
        {"QtGraphicalEffects", QString("1.14")},
        {"QtQuick.Shapes", QString("1.14")}};
}
...
std::unique_ptr<T> FigmaQml::construct(const QJsonObject& obj, const QString& targetDir, bool embedImages) const {
   ...
   if(!ensureDirExists(targetDir))
    ...
    for(const auto& c : components) {
      const auto component = FigmaParser::component(c->object(), m_flags, errorFunction, imageFunction, components);
      if(!ok || doCancel)
          return;
    ...
    for(const auto& c : canvases) {
        ...
        c.elements(), [&, this](const auto& f) {
            ...
            return FigmaParser::element(f, m_flags, errorFunction, imageFunction, components);
